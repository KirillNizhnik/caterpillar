...
class New_Feed extends \Cat\Core\Abstracts\Importer
{
    ...
    public function family($f, $class_id, $post_type, $rental=false)
    {
       ...
       if(is_wp_error( $xml ))
        ...
        $this->family_map[$f['id']] = $family->wp_term['term_id'];
    }
	...
	public function products($class, $noncurrent=false)
	{
		...
		{
            $this->product($p, $class->post_type, false, $noncurrent);
    ...
    public function product($p, $post_type, $rental=false, $noncurrent=false)
    {
        $url = $this->xml_base_url . $this->lang_code . '/' . $p['subfamily_id'] . '/' . $p['id'] . '_' . $this->lang_code . '.xml';
        $xml = $this->get_xml($url);
        ...
        $product->family    = get_term_by('id', $this->family_map[$p['family_id']], $post_type.$prefix.'_family');
        $product->family->family_id = $p['family_id'];

        $product->subfamily = get_term_by('id', $this->family_map[$p['subfamily_id']], $post_type.$prefix.'_family');
        $product->subfamily->family_id = $p['subfamily_id'];

        //if($product->family->slug === 'on-highway-trucks') return;
