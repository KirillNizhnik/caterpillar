...
class Templates
{
    ...
    if ( ! $wp_query->is_main_query() ) {
        ...
        {
            if( is_string($wp_query->query_vars['post_type'])
                && strpos($wp_query->query_vars['post_type'], 'cat_') === 0
            ){
                $wp_query->is_cat = true;

                if( strpos($wp_query->query_vars['post_type'], 'cat_new_') === 0
                    OR in_array($wp_query->query_vars['post_type'], array('cat_industry', 'cat_application'))
                ){
                ...
                if( strpos($wp_query->query_vars['post_type'], 'cat_used_') === 0 )
                {
        ...
        elseif( isset($object->taxonomy) )
        {
            if(  is_string($object->taxonomy)
                && strpos($object->taxonomy, 'cat_') === 0
            ){
                $wp_query->is_cat = true;

                if( strpos($object->taxonomy, 'cat_new_') === 0 ){
                    $wp_query->is_cat_new = true;
                }

                if( strpos($object->taxonomy, 'cat_used_') === 0 ){
                    $wp_query->is_cat_used = true;
                }
    ...
    public function set_template( $template )
    {
        ...
        if ( is_single() )
        {
            ...
            elseif( $object->post_type === 'cat_used_machine' )
            {
                $template = self::view('used/single');
            }
            elseif( strpos($object->post_type, 'cat_new_') === 0 )
            {
        ...
        elseif( is_tax() )
        {
            ...
            elseif( strpos($object->taxonomy, 'cat_used_') === 0 )
            {
    ...
    public function order_products_by_sort($wp_query)
    {
        if( !empty($wp_query->tax_query)
            AND ! empty($wp_query->tax_query->queries)
            AND ! is_admin()
        ){
            if( $wp_query->is_main_query()
                AND strpos($wp_query->tax_query->queries[0]['taxonomy'], 'cat_new_power_family') === 0
            ){
                $wp_query->set('orderby', 'title');
                $wp_query->set('order', 'ASC');
                $wp_query->set('posts_per_page', 9999);
            } elseif( $wp_query->is_main_query()
                AND strpos($wp_query->tax_query->queries[0]['taxonomy'], 'cat_new_') === 0
            ){
            ...
            if( $wp_query->is_main_query()
                AND strpos($wp_query->tax_query->queries[0]['taxonomy'], 'cat_used_') === 0
            ){
                $wp_query->set('posts_per_page', 9999);
            }
        }
