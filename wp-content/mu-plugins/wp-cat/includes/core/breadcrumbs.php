...
class Breadcrumbs
{
    ...
    {
        add_filter('wpseo_breadcrumb_links', array($this, 'breadcrumbs'), 9999);
...
//            $crumbs[$key]['url'] = str_replace('/used-equipment/', '/used/', $crumb['url']);
//        }
...
//            ),
    ...
    private function cat_new_single_crumbs($crumbs)
    {
        ...
        $terms = get_the_terms($post, 'family');
        if ($terms) {
...
//                ),
                ...
                array(
                    'text' => $lastTerm->name
                , 'url' => get_term_link($lastTerm->term_id, 'family')
                , 'allow_html' => true
                )
            );
