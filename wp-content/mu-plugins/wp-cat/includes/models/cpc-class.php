...
class CPC_Class
{
	...
	private function process_products($products=array(), $family=array())
	{
		foreach($products->product as $product)
		{
			...
			$pro = array();
            $pro['name']=  trim($product->name);
            $pro['nondisplayname']=  trim($product->nondisplayname);
			foreach($attrs as $k => $v)
				$pro[trim($k)] = trim($v);

            $pro['family_id'] = trim($family['id']);
            $pro['family_name'] = trim($family['name']);
			if(isset($family['parent']['id']))
			{
				$pro['sub_family_id'] = trim($family['parent']['id']);
                $pro['sub_family_name'] = trim($family['parent']['name']);
			}
