    ...
    public function import()
    {
        ...
        if (isset($xml->equipment)) {
            foreach ($xml->equipment as $item) {
                ...
                $modelForMapper = (string)$item->attributes()->{'model'};
                $familyForMapper = (string)$item->attributes()->{'product-family'};
...
//                    $product->families     = DSF_Family::create_from_xml($item);
                    ...
                    $product->modelForMapper = $modelForMapper;
                    $product->familyForMapper = $familyForMapper;
                    // save the data
                    $product->save();
    ...
    private function remove_old_items()
    {
        ...
        $imported = $wpdb->get_results(
            "SELECT p.ID
             ...
             WHERE post_type = 'used-equipment'
                   AND post_modified_gmt < '$this->import_start_time'
                   AND m.meta_key = 'used_status'
        ...
        if ($imported) {
            ...
            foreach ($imported as $i) {
                ...
                $fam = wp_get_object_terms($i->ID, 'used-family');
                $fam_id = [];
                foreach ($fam as $item) {
                    $fam_id[] = $item->term_id;
                }
                update_post_meta($i->ID, 'array_fam', $fam_id);
                wp_set_post_terms($i->ID, [], 'used-family');
                update_post_meta($i->ID, 'used_status', 'not_available');
